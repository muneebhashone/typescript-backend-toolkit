...
export const handleCreateTypeOfPlace = async (
  ...
  try {
    ...
    return successResponse(res, 'TypeOfPlace created successfully', newTypeOfPlace);
  } catch (err) {
    return errorResponse(res, (err as Error).message);
  }
};
...
export const handleUpdateTypeOfPlace = async (
  req: Request<TypeOfPlaceIdSchemaType, never, TypeOfPlaceCreateOrUpdateSchemaType>,
  res: Response,
) => {
  try {
    const updatedTypeOfPlace = await updateTypeOfPlace(req.body, {
      id: req.params.id,
    });
...
export const handleDeleteTypeOfPlace = async (
  ...
  try {
    await deleteTypeOfPlace({ id: req.params.id });
